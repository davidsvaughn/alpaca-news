...
# ---------------------------------------------------------------------------
# Test: cost_bps deduction (simulates what app.py does)
# ---------------------------------------------------------------------------


class TestCostBpsDeduction:
    """Verify transaction cost deduction logic (applied in app.py layer)."""

    @staticmethod
    def _apply_cost(results: list[BacktestResult], cost_bps: float):
        """Replicate the cost deduction from app.py."""
        if cost_bps > 0:
    ...
    def test_cost_reduces_pnl(self):
        """10 bps cost should reduce pnl_pct by 0.10 percentage points."""
        r = _make_result(pnl_pct=2.0, bars_held=390)
        self._apply_cost([r], cost_bps=10)
        assert r.pnl_pct == pytest.approx(1.90, abs=0.001)

    def test_cost_adjusts_entry_price(self):
        """10 bps cost should increase entry_price by 0.10%."""
        r = _make_result(pnl_pct=2.0, bars_held=390, entry_price=100.0)
        self._apply_cost([r], cost_bps=10)
        assert r.entry_price == pytest.approx(100.10, abs=0.01)

    def test_zero_cost_no_change(self):
        """cost_bps=0 should leave everything unchanged."""
        r = _make_result(pnl_pct=2.0, bars_held=390, entry_price=100.0)
        self._apply_cost([r], cost_bps=0)
        assert r.pnl_pct == 2.0
        assert r.entry_price == 100.0

    def test_cost_on_losing_trade(self):
        """Cost should make a losing trade even worse."""
        r = _make_result(pnl_pct=-1.0, bars_held=390)
        self._apply_cost([r], cost_bps=10)
        assert r.pnl_pct == pytest.approx(-1.10, abs=0.001)

    def test_cost_skip_none_pnl(self):
        """Trades with pnl_pct=None should keep None after cost application."""
        r = _make_result(pnl_pct=1.0, bars_held=390)
        r.pnl_pct = None
        self._apply_cost([r], cost_bps=10)
        assert r.pnl_pct is None

    def test_cost_reduces_ann_a(self):
        """Transaction costs should reduce annualized returns."""
        results_raw = [_make_result(pnl_pct=1.0, bars_held=390) for _ in range(5)]
        results_cost = [_make_result(pnl_pct=1.0, bars_held=390) for _ in range(5)]
        self._apply_cost(results_cost, cost_bps=10)

        stats_raw = compute_ann_a(results_raw)
        stats_cost = compute_ann_a(results_cost)
        assert stats_raw is not None and stats_cost is not None
        assert stats_cost["ann"] < stats_raw["ann"]

    def test_cost_can_flip_sign(self):
        """A marginally profitable trade should go negative with sufficient cost."""
        r = _make_result(pnl_pct=0.05, bars_held=390)
        self._apply_cost([r], cost_bps=10)  # 0.10 > 0.05
        assert r.pnl_pct < 0
